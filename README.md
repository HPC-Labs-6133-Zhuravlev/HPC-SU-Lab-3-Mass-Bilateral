...
spatial_weight = exp(-(dx^2 + dy^2) / (2 * sigma_d^2))
range_weight   = exp(-(I_neighbor - I_center)^2 / (2 * sigma_r^2))
```

где `dx`, `dy` — смещение соседнего пикселя относительно центрального пикселя,  
`I_neighbor` — яркость соседнего пикселя,  
`I_center` — яркость центрального пикселя.
...
![Результат для 1-го изображения](visual_image_1.png)

![Результат для 2-го изображения](visual_image_2.png)
